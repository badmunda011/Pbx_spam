...
from SHUKLASPAM.data import BAD
...
async def _(event):
    ...
    if check in BAD_RAID:
        await asyncio.sleep(0.1)
        await event.client.send_message(
            entity=event.chat_id,
            message="""{}""".format(choice(BAD)),
    ...
    if e.sender_id == enums.ChatMemberStatus.ADMINISTRATOR or enums.ChatMemberStatus.OWNER:
        ...
        try:
            ...
            if user_id in SHASHANK:
                ...
                if check not in BAD_RAID:
                    BAD_RAID.append(check)
